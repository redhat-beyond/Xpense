...
class HouseFactory(DjangoModelFactory):
    ...
    city = factory.LazyAttribute(
        lambda x: random.choice(City.objects.filter(country=x.country)))
    parent_profession_1 = factory.LazyAttribute(
        lambda x: random.choice([tup[1] for tup in Job.choices]))
    parent_profession_2 = factory.LazyAttribute(
        lambda x: random.choice([tup[1] for tup in Job.choices]))
